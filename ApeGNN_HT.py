...
class GraphConv(nn.Module):
    ...
    def forward(self, user_embed, item_embed, mess_dropout=True, edge_dropout=True):
        ...
        agg_embed = all_embed
        u_weight = (torch.exp(-self.user_t).to(self.dev) * torch.pow(self.user_t, torch.FloatTensor([0]).to(self.dev))
                    .to(self.dev) / torch.FloatTensor([factorial(0)]).to(self.dev))
        i_weight = (torch.exp(-self.item_t).to(self.dev) * torch.pow(self.item_t, torch.FloatTensor([0]).to(self.dev))
                    .to(self.dev) / torch.FloatTensor([factorial(0)]).to(self.dev))
        ...
        for k in range(1, self.n_hops + 1):
            ...
            user_embedds = user_embedds * (torch.exp(-self.user_t).to(self.dev) *
                                           torch.pow(self.user_t, torch.FloatTensor([k]).to(self.dev)).to(self.dev)
                                           / torch.FloatTensor([factorial(k)]).to(self.dev))
            item_embedds = item_embedds * (torch.exp(-self.item_t).to(self.dev) *
                                           torch.pow(self.item_t, torch.FloatTensor([k]).to(self.dev)).to(self.dev)
                                           / torch.FloatTensor([factorial(k)]).to(self.dev))
